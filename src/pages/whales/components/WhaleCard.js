...
import styles from "./WhaleCard.module.scss";

function WhaleCard(props) {
  const { results } = props;
  ...
  return (
    <Row>
      {results.map((row) => {
        ...
        return (
          <Col xl={4} className="mb-4" key={row.slug}>
            <Link
              to={`/whales/${row.slug}/`}
              key={row.protocol}
            ...
            >
              <div className={styles.boxHeader}>
                <div className={classnames(styles.boxTitle, "mb-4")}>
                  <CryptoIcon name={row.slug} size="3rem" />
...
export default withErrorBoundary(WhaleCard);
