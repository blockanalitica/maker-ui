...
function NewSupplyPerVaultTypeCard(props) {
  const { data } = props;
  if (!data) {
    return null;
  }
  ...
  return (
    <Card title="New Vault DAI Supply per Vault Type">
      <p className="mb-4">New DAI supply inflow split by ilk.</p>
      <Graph series={series} type="bar" options={options} />
    </Card>
  );
}

export default withErrorBoundary(NewSupplyPerVaultTypeCard);
